...
function move1(obj,iTarget){
	...
	obj.timer=setInterval(function(){
		...
		speed*= 0.91;	
		l+=speed;
		obj.style.WebkitTransform='perspective(800px) rotateX('+Math.round(l)+'deg)';
		obj.style.msTransform='perspective(800px) rotateX('+Math.round(l)+'deg)';
		obj.style.transform='perspective(800px) rotateX('+Math.round(l)+'deg)';			
		if(Math.abs(speed)<1) speed=0;
		if(obj.style.transform=='perspective(800px) rotateX(0deg)'){
			l=0;
		}
		if(l==0&&speed==0){
			clearInterval(obj.timer);
